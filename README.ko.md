...
저장된 배포가 없으면 바로 앱 주소를 입력합니다. 로그인은 필요할 때 브라우저로 연결하고,
프로젝트가 하나면 자동 선택합니다. 기본 설정은 포트 80, 24시간 후 중지, 내 공인 IPv4만
접속 허용입니다. 설정 화면에서 Enter로 진행하거나 `port`·`runtime`·`ip`를 골라
필요한 항목만 바꿉니다. 공인 IPv4 조회에는 `api.ipify.org`를 사용하며 실패하면 직접
입력합니다. PC에 `curl`을 설치할 필요는 없습니다.

기본 경로는 **앱 주소 입력 → 설정 화면 Enter → Terraform 계획 최종 승인**입니다.
설정 화면에서 진행하면 별도 배포 폴더와 설정을 만들고 필요한 Compute API를 활성화합니다.
준비용 `yes`는 없으며, 서버를 만들기 전에 실제 계획을 보고 최종 승인합니다.
비용 이메일 알림도 자동 설정합니다. 상시 운영은 `runtime`에서 선택할 수 있습니다.

배포 경로를 사용자 설정 디렉터리에 저장하므로, 다음부터는 어느 폴더에서든 프로그램을
실행해 배포를 선택하고 관리·삭제할 수 있습니다. 삭제는 대상과 계획을 다시 확인합니다.
목록에는 경로만 저장하며 서버 상태나 비밀번호는 저장하지 않습니다. 상태 파일이 있는
원래 배포 폴더는 계속 보관해야 합니다. 오래된/삭제된 폴더는 목록에서 건너뜁니다.
`GCP_FREE_DEPLOY_HOME`으로 목록 저장 위치를 변경할 수 있습니다.

접근 가능한 프로젝트가 없으면 새 프로젝트 생성을 제안합니다. 사용 가능한 결제 계정을
선택하고 생성할 이름·ID·결제 연결을 확인해 승인하면 프로젝트를 생성하고 연결합니다.
결제 계정 개설·결제 수단 등록·권한 부여 및 Terraform·Google Cloud CLI 설치는 사용자가
준비합니다. 회사 정책으로 생성이 제한되면 안내 후 중단합니다.

새 배포 안내는 선택한 결제 계정에 연결된 프로젝트 전체의 현재 VM(중지 포함), 디스크,
예약 외부 IP를 조회합니다. 기존 무료 대상 VM 형태도 표시하지만 **실제 무료 적용 여부나
이번 달 남은 무료량을 뜻하지 않습니다.** 조회 실패·비활성 API는 사용량 미확인으로
표시합니다. 기존 자원이나 미확인 항목이 있으면 기본적으로 중단하고, 명시적으로
`allow-cost-risk`를 입력한 경우에만 계속합니다. 프로젝트 신규 생성도 이 점검 뒤에
수행합니다. 점검은 API 활성화나 자원 변경을 하지 않습니다. 과거 삭제된 자원의 사용량,
송신량, 다른 서비스와 실제 청구 내역은 점검 범위 밖입니다.

생성 후 결제 연결에 실패하면 프로젝트를 임의로 삭제하지 않고 ID와 복구 안내를 보여줍니다.
아래는 설정 파일을 직접 관리하는 방법입니다.
