...
## 안내형 시작의 계정 전체 점검과 프로젝트 생성

`start`는 새 배포 전에 선택한 결제 계정의 연결 프로젝트 목록을 가져와 현재 VM·디스크·
예약 외부 IP를 조회합니다. 중지된 VM과 연결되지 않은 디스크도 포함합니다. 계정/프로젝트
조회 권한 부족, API 비활성, 불완전한 응답은 사용량 0으로 처리하지 않습니다.
기존 자원이나 미확인 항목이 있으면 중단하며, 사용자가 `allow-cost-risk`를 직접 입력해야
계속할 수 있습니다. 기존 자원은 자동으로 삭제하거나 이전하지 않습니다.

프로젝트 목록이 비어 있으면 생성 제안을 표시하고, 열린 결제 계정 선택·표시 이름 입력·
실제 생성할 ID와 결제 연결 승인 후 위 점검을 수행합니다. 점검을 통과하거나 위험을
명시적으로 수용한 뒤 생성합니다. 결제 계정 개설과 결제 수단 등록은 자동화하지 않습니다.
회사 정책이나 권한 때문에 생성/연결에 실패하면 안내하고 중단하며, 이미 생성된 프로젝트는
삭제하지 않고 복구할 ID를 남깁니다. 기존 프로젝트가 결제 미연결이면 안내 후 중단합니다.

이 기능은 현재 배포와 겹치는 Compute 자원을 찾습니다. 과거 삭제한 자원 사용량·월간
트래픽·다른 서비스의 무료 한도나 실제 청구는 확인하지 않습니다. 무료 VM 형태를 찾았다는
것과 해당 VM이 무료로 청구되고 있다는 것은 별개입니다. 일반 `up`의 기존 프로젝트 VM
중복 검사는 유지하며, 계정 전체 스캔은 안내형 새 배포 경로에서 실행됩니다.
